...
def test_DerivativesDataSink_build_path(tmp_path, source, input_files, entities, expectation):
    ...
    if isinstance(expectation, str):
        assert Path(dds.outputs.out_file).relative_to(tmp_path) \
            == Path("niworkflows") / expectation
    else:
        for out, exp in zip(dds.outputs.out_file, expectation):
            assert Path(out).relative_to(tmp_path) == Path("niworkflows") / exp
    ...
    if isinstance(expectation, str):
        assert Path(dds.outputs.out_file).relative_to(tmp_path) \
            == Path("niworkflows") / expectation.replace("_run-1", "")
    else:
        for out, exp in zip(dds.outputs.out_file, expectation):
            assert Path(out).relative_to(tmp_path) \
                == Path("niworkflows") / exp.replace("_run-1", "")
