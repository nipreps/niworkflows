...
@pytest.mark.parametrize('source,input_files,entities,expectation', [
    (T1W_PATH, ["anat.nii.gz"], {"desc": "preproc"},
        "sub-100185/anat/sub-100185_desc-preproc_T1w.nii.gz"),
    (T1W_PATH, ["anat.nii.gz"], {"desc": "preproc", "space": "MNI"},
        "sub-100185/anat/sub-100185_space-MNI_desc-preproc_T1w.nii.gz"),
    (T1W_PATH, ["anat.nii.gz"], {"desc": "preproc", "space": "MNI", "resolution": "native"},
        "sub-100185/anat/sub-100185_space-MNI_desc-preproc_T1w.nii.gz"),
    (T1W_PATH, ["anat.nii.gz"], {"desc": "preproc", "space": "MNI", "resolution": "high"},
        "sub-100185/anat/sub-100185_space-MNI_res-high_desc-preproc_T1w.nii.gz"),
    (T1W_PATH, ["tfm.txt"], {"from": "fsnative", "to": "T1w", "suffix": "xfm"},
        "sub-100185/anat/sub-100185_from-fsnative_to-T1w_mode-image_xfm.txt"),
    (T1W_PATH, ["tfm.h5"], {"from": "MNI152NLin2009cAsym", "to": "T1w", "suffix": "xfm"},
        "sub-100185/anat/sub-100185_from-MNI152NLin2009cAsym_to-T1w_mode-image_xfm.h5"),
    (T1W_PATH, ["anat.nii.gz"], {"desc": "brain", "suffix": "mask"},
        "sub-100185/anat/sub-100185_desc-brain_mask.nii.gz"),
    (T1W_PATH, ["anat.nii.gz"], {"desc": "brain", "suffix": "mask", "space": "MNI"},
        "sub-100185/anat/sub-100185_space-MNI_desc-brain_mask.nii.gz"),
    (T1W_PATH, ["anat.surf.gii"], {"suffix": "pial", "hemi": "L"},
        "sub-100185/anat/sub-100185_hemi-L_pial.surf.gii"),
    (T1W_PATH, ["aseg.nii", "aparc.nii"], {"desc": ["aseg", "aparcaseg"], "suffix": "dseg"},
...
def test_DerivativesDataSink_build_path(
    ...
    if isinstance(expectation, str):
        assert Path(dds.outputs.out_file).relative_to(tmp_path) \
            == Path(base) / expectation
    else:
        for out, exp in zip(dds.outputs.out_file, expectation):
    ...
    if isinstance(expectation, str):
        assert Path(dds.outputs.out_file).relative_to(tmp_path) \
            == Path(base) / expectation.replace("_run-1", "")
    else:
        for out, exp in zip(dds.outputs.out_file, expectation):
            assert Path(out).relative_to(tmp_path) \
                == Path(base) / exp.replace("_run-1", "")
