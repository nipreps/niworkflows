...
class SelectReference(SimpleInterface):
    ...
    def _run_interface(self, runtime):
        if isdefined(self.inputs.reference_image):
            ...
            _template_fmt = op.join(getters.get_dataset(self.inputs.template), '%dmm_%s.nii.gz')
            reference_image = _template_fmt % (resolution, self.inputs.reference)
            reference_mask = _template_fmt % (resolution, 'brainmask')
